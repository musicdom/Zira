...
function validRecipe(r){
  return r&&typeof r.name==='string'&&r.name.trim()&&typeof r.cat==='string'&&Array.isArray(r.filters)&&Array.isArray(r.ingredients)&&Array.isArray(r.steps);
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='GET'){
      return res.status(200).json({ok:true,recipes:current.recipes||[],user:{id:user.id,name:user.first_name||''}});
    }
    ...
    if(body.action==='save'){
      ...
      if(i>=0)list[i]=r;
        ...
        r.id=Math.max(80,...list.map(x=>Number(x.id)||0))+1;
        list.push(r);
      }
